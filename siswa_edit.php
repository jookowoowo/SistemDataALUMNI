...
// Bagian ini untuk mengambil data siswa yang akan diedit SAAT HALAMAN PERTAMA KALI DIMUAT
if (isset($_GET['id']) && !empty($_GET['id'])) {
    $id_siswa = mysqli_real_escape_string($conn, $_GET['id']);
    $sql_fetch = "SELECT * FROM siswa WHERE id_siswa = '$id_siswa'";
    $result_fetch = mysqli_query($conn, $sql_fetch);

    if ($result_fetch && mysqli_num_rows($result_fetch) > 0) {
        $siswa_data = mysqli_fetch_assoc($result_fetch);
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id_siswa'])) {
    ...
    if (empty($error_message)) {
        ...
        if (mysqli_query($conn, $sql_update)) {
            header("Location: alumni_list.php?status=success_edit_siswa");
            exit(); 
        } else {
...
<div class="container mx-auto p-4 mt-8">
    <h2 class="text-3xl font-bold text-gray-900 mb-6 text-center">Edit Data Siswa</h2>
    ...
    <?php if ($siswa_data): // Pastikan data siswa sudah terambil sebelum menampilkan form ?>
    <div class="bg-white shadow-lg rounded-lg p-6">
        <form action="siswa_edit.php" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="id_siswa" value="<?php echo htmlspecialchars($siswa_data['id_siswa']); ?>">
            <input type="hidden" name="current_foto" value="<?php echo htmlspecialchars($siswa_data['foto']); ?>">

            <div class="mb-4">
                <label for="nama" class="block text-gray-700 text-sm font-bold mb-2">Nama Lengkap:</label>
                <input type="text" name="nama" id="nama" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?php echo htmlspecialchars($siswa_data['nama']); ?>" required>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="email" name="email" id="email" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?php echo htmlspecialchars($siswa_data['email']); ?>" required>
            </div>
            <div class="mb-4">
                <label for="kelas" class="block text-gray-700 text-sm font-bold mb-2">Kelas:</label>
                <input type="text" name="kelas" id="kelas" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?php echo htmlspecialchars($siswa_data['kelas']); ?>" required>
            </div>
            <div class="mb-4">
                <label for="alamat" class="block text-gray-700 text-sm font-bold mb-2">Alamat:</label>
                <textarea name="alamat" id="alamat" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" rows="4" required><?php echo htmlspecialchars($siswa_data['alamat']); ?></textarea>
            </div>
            <div class="mb-4">
                ...
                <select name="id_jurusan" id="id_jurusan" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                    ...
                    while($row_jurusan = mysqli_fetch_assoc($jurusan_result)): ?>
                        <option value="<?php echo htmlspecialchars($row_jurusan['id_jurusan']); ?>" <?php echo ($siswa_data['id_jurusan'] == $row_jurusan['id_jurusan'] ? 'selected' : ''); ?>>
            ...
            <div class="mb-4">
                ...
                <?php if (!empty($siswa_data['foto'])): ?>
                    <img src="uploads/<?php echo htmlspecialchars($siswa_data['foto']); ?>" alt="Foto Saat Ini" class="w-24 h-24 object-cover rounded-full mb-2">
                    <p class="text-xs text-gray-600 mb-2">File saat ini: <?php echo htmlspecialchars($siswa_data['foto']); ?></p>
                <?php endif; ?>
