...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $sql_select_foto = "SELECT foto FROM alumni WHERE id_alumni = '$id_alumni'";
    ...
    // Kemudian, hapus data alumni dari database
    $sql_delete = "DELETE FROM alumni WHERE id_alumni = '$id_alumni'";
    if (mysqli_query($conn, $sql_delete)) {
        // Redirect kembali ke halaman daftar alumni dengan pesan sukses
        header("Location: alumni_list.php?status=success_delete_alumni");
        exit(); 
    } else {
    ...
    // Tampilkan pesan error jika ID alumni tidak disediakan atau tidak valid
    echo "<div class='container mx-auto p-4 mt-8 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative' role='alert'>";
    echo "ID Alumni tidak valid atau tidak disediakan.";
    echo "</div>";
    exit(); 
}
